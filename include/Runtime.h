...
class Runtime {
 ...
 public:
  Runtime(int num_compute_threads, int num_io_threads, uint64_t io_buffer_size)
    ...
    _io_engine = new IoEngine(num_io_threads, num_compute_threads, io_buffer_size / num_io_threads,
                              _fetched_tasks);

    _compute_engine = new ComputeEngine(1, num_compute_threads, _fetched_tasks);

    setRuntimeInstance(this);
  }

  ~Runtime() {
    double io_bw_in_gbps =
        _total_io_time > 0 ? (double)_total_accessed_io_bytes / _total_io_time / GB : 0.0;
    printf("# IO SUMMARY    : %'lu bytes, %8.5f sec, %4.2f GB/s\n", _total_accessed_io_bytes,
           _total_io_time, io_bw_in_gbps);
    printf("# SUMMARY       : %'lu edges accessed.\n", _total_accessed_edges);

    if (_io_engine) delete _io_engine;

    if (_compute_engine) delete _compute_engine;

    if (_pb_engine) delete _pb_engine;
  ...
  int getNumberOfComputeWorkers() const { return _num_compute_threads; }

  int getNumberOfIoWorkers() const { return _num_io_threads; }

  IoEngine* getIoEngine() { return _io_engine; }

  ComputeEngine* getComputeEngine() { return _compute_engine; }

  int getRound(void) const { return _round; }

  void incRound(void) { _round++; }

  void addAccessedIoBytes(uint64_t bytes) { _total_accessed_io_bytes += bytes; }

  void addAccessedEdges(uint64_t edges) { _total_accessed_edges += edges; }

  uint64_t getAccessedEdges() const { return _total_accessed_edges; }

  void addIoTime(double time) { _total_io_time += time; }
  ...
  void initBinning(float ratio) {
    ...
    _pb_engine = new PBEngine(1, num_bin_workers, num_acc_workers, _fetched_tasks);
  }

  PBEngine* getPBEngine() { return _pb_engine; }

  clock::time_point getStartTime() const { return _start_time; }
  duration getElapsedTime() const {
    return std::chrono::duration_cast<duration>(clock::now() - _start_time);
  }
 ...
 private:
  ...
  const clock::time_point _start_time;
};
